...
import "../User/StudentNavbar.css";
import logo from "../../assets/Student/Logo.png";
import { getFirebaseToken, onMessageListener } from "../User/Dashboard/firebase";
  ...
  useEffect(() => {
    ...
    getFirebaseToken().then((token) => {
      if (token) {
        ...
        // Here, you should save the FCM token to your backend for the user.
      }
    });
    ...
    <div className="navbar-container md:px-4 px-2 sticky top-0 w-full h-fit border-b-[1px] flex justify-between items-center bg-white">
      ...
      <div className="w-[40%] flex justify-end h-max">
        <div className="flex items-center">
          ...
          <div className="flex items-center hidden md:flex md:flex-row">
            <div className="relative flex items-center">
              {user.userImg ? (
                <img
                  className="rounded-full object-cover overflow-hidden md:w-[42px] md:h-[42px] w-[20px]"
                ...
                <div className="rounded-full bg-gray-600 flex items-start justify-center text-white font-bold text-xl md:w-[42px] md:h-[42px] w-[20px] h-[20px]">
                  {user.userName ? user.userName.charAt(0).toUpperCase() : ""}
                </div>
              )}
            </div>
