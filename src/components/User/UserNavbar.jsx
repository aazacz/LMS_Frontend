...
const UserNavbar = ({ toggleSidebar, isSidebarOpen }) => {
  ...
  const list = ["Subjects", "Courses", "Pricing", "Methodology", "Team", "Contact"];
  ...
  return (
    <div className='md:px-4 px-4 w-screen h-[7vh] md:h-[12vh] border-b-[1px] flex items-center md:py-0 relative transition-all duration-500'>
      ...
      <div className='hidden  w-[50%] md:grid grid-flow-row grid-cols-6 px-5'>
        {list.map((val, index) => (
          <h1 key={index} className='text-[#0066DE] font-plusjakartasans text-center cursor-pointer  md:text-base font-semibold'>
            {val}
          </h1>
        ))}
      </div>
