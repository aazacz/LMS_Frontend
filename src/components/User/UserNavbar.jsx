      ...
      <button className='w-[35px] bg-blue-700 h-[35px] flex justify-center items-center'>
        <IoSearch className='text-white' />
            ...
            <button className="md:hidden ml-auto mr-4" onClick={handleToggle}>
                <FaBars
                    className={`text-lg  ${show ? 'rotate-90 ' : ''}    transition-all duration-700`}
                />
            </button>

            {
                <div
                    className={` ${!show ? '-translate-y-[700px] ' : ''}  transition-all z-50 duration-700  absolute top-[7vh] right-0 w-full bg-white shadow-lg md:hidden `}
                >
                    ...
                    {list.map((val, index) => (
                        <h1
                            key={index}
                            className="text-[#0066DE] bg-white hover:text-lg font-plusjakartasans text-base font-semibold px-4 py-2 border-b"
                        >
                            {val}
                        </h1>
                    ))}
