      ...
      if (response.data) {
        ...
        setCourse({
          ...
          description:  response.data.description 
                        ? EditorState.createWithContent(convertFromRaw(JSON.parse(response.data.description)))
    ...
    if (selectedCourse) {
      setCourse((prevState) => ({
        ...
        description: selectedCourse.description ? 
                     EditorState.createWithContent(convertFromRaw(JSON.parse(selectedCourse.description)))
                    : EditorState.createEmpty(),
  ...
  const submitHandler = (e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        AdminAxiosInstance.post(`api/course/create`, formData).then((res) => {
  ...
  const handleImageChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        const response = await AdminAxiosInstance.put(
          `api/course/update-image/${courseId}`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
    ...
    <div className="w-full p-5 md:px-16 bg-slate-200 rounded-lg mt-2">
      ...
      <form className="space-y-6">
        ...
        <div className="flex flex-col ">
          ...
          <TextEditor editorState={course.description}  onEditorStateChange={onEditorStateChange}  />
