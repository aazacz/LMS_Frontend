...
const Sidebar = ({ isOpen }) => {
    ...
    const navLinks = [
        { title: "Dashboard", path: "/admin/home/dashboard", icon: <PiCirclesFourFill /> },
        { title: "Courses", path: "/admin/home/courses", icon: <PiCirclesFourFill /> },
        { title: "Course Structure", path: "/admin/home/courseStructure", icon: <PiCirclesFourFill /> },
        ...
        { title: "Settings", path: "/admin/home/settings", icon: <PiCirclesFourFill /> },
    ];
        ...
        <div className={`z-30 absolute md:sticky flex flex-col justify-between bottom-0 top-0 bg-white md:w-64 w-[50%] h-screen px-2 transition-transform duration-200 transform ${isOpen ? 'translate-x-0 md:translate-x-0 shadow-[rgba(0,0,15,0.5)_5px_0px_10px_-5px]' : '-translate-x-full md:translate-x-0 shadow-none md:shadow-[rgba(0,0,15,0.5)_5px_0px_10px_-5px]'}`}>
            ...
            <div>
                {navLinks.map((link, index) => (
                    <Link key={link.title} to={link.path} className={`flex items-center px-3 my-4 mb:mb-3 rounded-lg font-poppins p-2 text-gray-700 hover:bg-gray-200 ${location.pathname === link.path ? "bg-blue-200 text-blue-900" : "bg-transparent text-gray-500"}`}>
                        <PiCirclesFourFill className={`mr-2 text-xl ${location.pathname === link.path ? "text-white" : "text-gray-300"}`} />
