...
const ReusableTable = ({ columns, data, link,isPending }) => {
  ...
  const handleActive = async (activeId) => {
    ...
    try {
      ...
      await axios.patch(`${apiURL}api/diagnosis/active-diagnosis/${activeId}`)
        .then((response) => {
          console.log(response)
          if (response.data.message === "Diagnosis Test activated successfully") {
  ...
  const handleUnBlock = async (activeId) => {
    ...
    try {
      ...
      await axios.patch(`${apiURL}api/students/unblock-students/${activeId}`)
        .then((response) => {
          console.log(response)
      ...
      <tr key={indexrow}>
        {columns.map((column, index) => (
          <td key={index}>
            ...
            {column.field === "marks" ? (
              <div className="font-semibold text-center">
                {row.questions.length * row.positiveMark}
              </div>
            ) : null}
            ...
            {column.field === "status" ? (
              <div className="action-container">
                {Active ? (
                  <div
                    onClick={() => handleDiagnosisTestActive("active", row._id)}
                  ...
                  <div
                    onClick={() => handleDiagnosisTestActive("inactive", row._id)}
