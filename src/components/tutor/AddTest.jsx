...
const AddTest = () => {
    ...
    const handleIsCorrectChange = (e, questionIndex, choiceIndex) => {
        ...
        console.log(checked)
        const updatedQuestions = [...test.questions]
        updatedQuestions[questionIndex].choices[choiceIndex].isCorrect =
            !checked
        setTest({ ...test, questions: updatedQuestions })
        getBackgroundColor()
    }
    ...
    const validateTest = (test) => {
        ...
        if (!test.positiveMark || isNaN(test.positiveMark)) {
            // Check for valid number in positiveMark
            return false
        }
        if (!test.negativeMark || isNaN(test.negativeMark)) {
            // Check for valid number in negativeMark
            return false
        }
        if (!test.timeSlot) {
            // Check for existence of timeslot
            return false
        }
        ...
        for (let question of test.questions) {
            ...
            for (let choice of question.choices) {
                if (!choice.choiceText || !choice.choiceText.trim()) {
                    // Check for empty or whitespace-only choice text
                    return false
                }
            }
        }
                    ...
                    >
                        ...
                        {question.choices.map((choice, cIndex) => (
                            ...
                            >
                                ...
                                <div
                                    className={`flex w-20 mt-2 py-2 justify-between  shadow-[0_3px_10px_rgb(0,0,0,0.2)] ${getBackgroundColor(
                                        choice.isCorrect
                                    )} px-2 rounded-md  `}
                                >
                                    <input
